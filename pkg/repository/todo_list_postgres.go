...
func (r *TodoListPostgres) Create(userId int, list todo.TodoList) (int, error) {
	...
	/* list.Title и list.Description — это значения, которые подставляются в параметры $1 и $2 SQL-запроса

	QueryRow — метод объекта транзакции, который выполняет SQL-запрос, ожидающий одну строку результата.
	*/
	...
	return id, tx.Commit()
	//	 Команда tx.Commit() завершает транзакцию и фиксирует (коммитит) в
	//	 се изменения
}
...
func (r *TodoListPostgres) Delete(userId, listId int) error {
	...
	//	 Exec делает запрос в бд и подставляет значения
	return err
}
